...
const convertToTrueSolarTime = (year, month, day, hour, minute, longitude) => {
  const standardDate = new Date(year, month - 1, day, hour, minute);
  const longOffset = (longitude - 120) * 4;
  const eot = calculateEoT(standardDate);
  const totalOffsetMinutes = longOffset + eot;
  const trueSolarDate = new Date(standardDate.getTime() + totalOffsetMinutes * 60 * 1e3);
  return { date: trueSolarDate, eot, longOffset };
};
const getMonthStem = (yearGan, monthZhi) => {
...
const getHourStem = (dayGan, hourZhi) => {
